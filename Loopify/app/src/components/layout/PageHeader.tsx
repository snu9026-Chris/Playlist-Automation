 ...
 * 동작:
 *   - sticky top-0 z-30: 스크롤하면 상단에 붙어 있도록
 *   - -mx-8 -mt-8 px-8 pt-8 pb-4: main의 p-8 패딩을 가로/세로로 침범해서 sticky 시 카드가 바 뒤로 깔끔하게 흘러가도록
 *   - 배경 + backdrop-blur + 하단 border로 sticky 상태에서 시각적으로 분리
...
export default function PageHeader({
  ...
  return (
    <div
      className={`sticky top-0 z-30 -mx-8 -mt-8 px-8 pt-8 pb-4 bg-pearl-100/90 backdrop-blur-sm border-b border-pearl-200/60 ${className}`}
    >
      {children}
    </div>
  );
